"""
Wrapper of ODL operators to pytorch-style functions (with built-in AD)

Taken from
https://github.com/Subhadip-1/data_driven_convex_regularization/blob/main/torch_wrapper.py
"""
import numpy as np
import torch
import odl
...
class OperatorFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if not operator.is_linear:
            ...
            # TODO(kohr-h): remove `copy_if_zero_strides` when NumPy 1.16.0
            # is required
            input_arr = copy_if_zero_strides(ctx.saved_tensors[0].detach().cpu().numpy())
        ...
        # Check if `grad_output` is consistent with `extra_shape` and `op_out_shape`
        if grad_output_arr.shape != extra_shape + op_out_shape:
            raise ValueError('expected tensor of shape {}, got shape {}'''.format(extra_shape + op_out_shape, grad_output_arr.shape))
        ...
        if extra_shape:
            ...
            # Stack results, reshape to the expected output shape and enforce correct dtype
            ...
            if operator.is_linear:
                result_arr = np.asarray(operator.adjoint(grad_output_arr)).astype(op_in_dtype, copy=False)
            else:
                result_arr = np.asarray(operator.derivative(input_arr).adjoint(grad_output_arr)).astype(op_in_dtype, copy=False)
...
class OperatorModule(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if in_ndim <= op_in_ndim or in_shape[-op_in_ndim:] != op_in_shape:
            shp_str = str(op_in_shape).strip('()')
            raise ValueError('input tensor has wrong shape: expected (N, *, {}), got {}'''.format(shp_str, in_shape))
    ...
    def __repr__(self):
        ...
        return '{}({}) ({} -> {})'.format(self.__class__.__name__, op_name, op_in_shape, op_out_shape)
